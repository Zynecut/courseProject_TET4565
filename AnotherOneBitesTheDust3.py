...
def wind_surplus_constraint(m, s):
    return m.wind_sur[s] >= m.wind_DA - m.P_wind[s]
...
def load_balance_RT(m, s):
    return m.hydro_RT[s] + m.wind_RT[s] + m.rationing["Load 1", s] == m.demand["Load 1"]

"""Objective Function"""
def ObjFunction(m):
    production_cost_DA = m.MC['nuclear'] * m.nuclear_DA + m.MC['hydro'] * m.hydro_DA + m.MC['wind'] * m.wind_DA
...
def modelSetup_1(data):
    ...
    m.wind_DA =     pyo.Var(bounds=limit_wind_DA, within=pyo.NonNegativeReals)
